...
class SessionScreen(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.classroom_map = {}  # {"display_text": classroom_id}
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # Trường số 2: Dropdown chọn Lớp Học
        ctk.CTkLabel(form_scroll, text="Lớp Học", font=(FONT_FAMILY, 12, "bold"), text_color=THEME_COLORS["text_muted"]).pack(anchor="w", pady=(5, 2))
        self.cmb_classroom = ctk.CTkComboBox(
            form_scroll, 
            values=["-- Đang tải danh sách lớp --"],
            ...
            dropdown_hover_color=THEME_COLORS["bg_card_hover"],
        ...
        self.cmb_classroom.pack(fill="x", pady=(0, 15))
        ...
        # Trường số 3: Chọn hoặc nhập ngày học
        ...
        # Trường số 4: Nhập thời gian bắt đầu tiết học
        ...
        # Trường số 5: Nhập thời gian kết thúc tiết học
        ...
        # Tải danh sách lớp học từ DB
        self.load_classrooms()

    def load_classrooms(self):
        """Tải danh sách lớp học từ Controller và đổ vào dropdown"""
        classrooms = self.controller.load_classrooms()
        self.classroom_map = {}
        
        if not classrooms:
            self.cmb_classroom.configure(values=["-- Chưa có lớp học nào --"])
            self.cmb_classroom.set("-- Chưa có lớp học nào --")
            return

        display_values = []
        for cls in classrooms:
            display = f"{cls['class_code']} - {cls['class_name']}"
            display_values.append(display)
            self.classroom_map[display] = cls["classroom_id"]
        
        self.cmb_classroom.configure(values=display_values)
        self.cmb_classroom.set(display_values[0])

    def refresh_and_load_data(self):
        """Gọi lại khi chuyển tab vào màn hình này"""
        self.load_classrooms()
    ...
    def handle_confirm_session(self):
        ...
        # Lấy classroom_id từ dropdown đã chọn
        selected_display = self.cmb_classroom.get()
        classroom_id = self.classroom_map.get(selected_display)
    ...
    def handle_reset_form(self):
        ...
        if self.classroom_map:
            first_key = list(self.classroom_map.keys())[0]
            self.cmb_classroom.set(first_key)
